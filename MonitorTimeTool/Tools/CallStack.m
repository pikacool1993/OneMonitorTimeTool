...
+ (NSString *)callStackWithType:(CallStackType)type {
    if (type == CallStackTypeAll) {
        ...
        for (int i = 0; i < thread_count; i++) {
            //当前执行的指令
            [reStr appendString:stackOfThread(threads[i])];
        }
        //task info
        NSString *memStr = @"";
...
//        NSLog(@"%@%@",memStr,reStr);
        ...
        return [reStr copy];
    } else if (type == CallStackTypeMain) {
        //主线程
        NSString *reStr = stackOfThread((thread_t)_mainThreadId);
        assert(vm_deallocate(mach_task_self(), (vm_address_t)_mainThreadId, 1 * sizeof(thread_t)) == KERN_SUCCESS);
//        NSLog(@"%@",reStr);
        return [reStr copy];
    } else {
        //当前线程
...
//                    NSLog(@"%@",reStr);
        ...
        [nsthread setName:originName];
        reStr = stackOfThread(mach_thread_self());
//        NSLog(@"%@",reStr);
        return [reStr copy];
    }
    return @"";
...
NSString *stackOfThread(thread_t thread) {
    ...
    mach_msg_type_number_t state_count = threadStateCountByCPU();
    kern_return_t kr = thread_get_state(thread, threadStateByCPU(), (thread_state_t)&machineContext.__ss, &state_count);
    if (kr != KERN_SUCCESS) {
        return [NSString stringWithFormat:@"Fail get thread: %u", thread];
    }
    // 通过指令指针来获取当前指令地址
    const uintptr_t instructionAddress = machInstructionPointerByCPU(&machineContext);
    buffer[i] = instructionAddress;
    ++i;
    
    //
    uintptr_t linkRegisterPointer = machThreadGetLinkRegisterPointerByCPU(&machineContext);
    ...
    // 通过栈基址指针获取当前栈帧地址
    const uintptr_t framePointer = machStackBasePointerByCPU(&machineContext);
    if (framePointer == 0 || memCopySafely((void *)framePointer, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
        return @"Fail frame pointer";
    }
    for (; i < 32; i++) {
        buffer[i] = stackFrame.return_address;
        if (buffer[i] == 0 || stackFrame.previous == 0 || memCopySafely(stackFrame.previous, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
            break;
        }
    }
    ...
    symbolicate(buffer, symbolicated, stackLength, 0);
    for (int i = 0; i < stackLength; ++i) {
        [reStr appendFormat:@"%@", outputLog(i, buffer[i], &symbolicated[i])];
    }
    [reStr appendFormat:@"\n"];
    return reStr;
}
...
NSString* outputLog(const int entryNum, const uintptr_t address, const Dl_info* const dlInfo) {
...
kern_return_t memCopySafely(const void *const src, void *const dst, const size_t byteSize) {
    vm_size_t bytesCopied = 0;
    return vm_read_overwrite(mach_task_self(), (vm_address_t)src, (vm_size_t)byteSize, (vm_address_t)dst, &bytesCopied);
}

#pragma mark - Symbolicate
void symbolicate(const uintptr_t* const stackBuffer, Dl_info* const symbolsBuffer, const int stackLength, const int skippedEntries) {
    int i = 0;
    ...
    for (; i < stackLength; i++) {
        smDladdr(instructionAddressByCPU(stackBuffer[i]), &symbolsBuffer[i]);
    }
}
bool smDladdr(const uintptr_t address, Dl_info* const info) {
    ...
    info->dli_saddr = NULL;
    //根据地址获取是哪个 image
    const uint32_t idx = dyldImageIndexFromAddress(address);
    if (idx == UINT_MAX) {
        return false;
    }
    /*
    ...
    uintptr_t cmdPointer = cmdFirstPointerFromMachHeader(machHeader);
    if (cmdPointer == 0) {
        return false;
    }
...
uint32_t dyldImageIndexFromAddress(const uintptr_t address) {
...
uintptr_t cmdFirstPointerFromMachHeader(const struct mach_header* const machHeader) {
...
uintptr_t segmentBaseOfImageIndex(const uint32_t idx) {
    ...
    uintptr_t cmdPtr = cmdFirstPointerFromMachHeader(machHeader);
    if (cmdPtr == 0) {
        return 0;
    }
 ...
 */
uintptr_t machStackBasePointerByCPU(mcontext_t const machineContext) {
...
uintptr_t machInstructionPointerByCPU(mcontext_t const machineContext) {
...
uintptr_t instructionAddressByCPU(const uintptr_t address) {
#if defined(__arm64__)
...
mach_msg_type_number_t threadStateCountByCPU() {
#if defined(__arm64__)
...
*/
thread_state_flavor_t threadStateByCPU() {
#if defined(__arm64__)
...
uintptr_t machThreadGetLinkRegisterPointerByCPU(mcontext_t const machineContext) {
#if defined(__i386__)
    return 0;
#elif defined(__x86_64__)
    return 0;
